...
with st.sidebar:
    # Menambahkan logo 
    st.image("dashboard/bikesharingboardimage.png")
...
st.header('Bike Sharing Statistic Board')

# Statistik penyewaan sepeda berdasarkan hari Libur dan hari kerja
st.subheader("Statistik jumlah penyewaan sepeda Berdasarkan Hari Libur dan Hari Kerja")
col_holiday, col_workingday = st.columns([1, 1])

with col_holiday:
    fig, ax = plt.subplots()
    fig.patch.set_facecolor('#f5f5f5')  # Mengatur warna latar belakang figure
    ax.set_facecolor('#e0f7fa')  # Mengatur warna latar belakang area plot
...
st.caption("Copyright "+ str(datetime.date.today().year) + " " + "Fitri Ning Alisia")
